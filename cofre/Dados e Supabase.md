...
- **`endodirect_member_content()`** entrega: `acervo_totais`, `member`, `acessos`, `cursos`, `adm_avisos` (avisos do professor + **radar janelado nos 200 mais recentes**, ambos sem o que está em `radar_hidden`; o resto do radar vem por `endodirect_mural_radar_more`), `radar_hidden`, **provas** (assinante = tudo que **não é TEEM** → Endodirect + residências; EndoTEEM = Endodirect+TEEM; degustação = 50 do Endodirect), `podcasts`/`mm_shared`/`fc_shared`/`adm_cursos` gated por plano/tier. **~5,3 MB p/ Gold** (24/09/2026; era 12,3 MB — ver [[Decisões]] 2026-09-24). ⚠️ **Desde 24/09 NÃO entrega `diretrizes`/`diretrizes_temas`** (vinham em duplicata com `member_resumos` e eram descartadas pelo cliente). Corpo completo versionado em `supabase/member-content-mais-leve.sql` — 🧨 **patch parcial nesta função já perdeu duas migrações** (janela de 23/07 e filtro de 03/08, derrubados pela `member_content_capa_do_curso` de 07/08): antes de reescrever, ler o `prosrc`; depois, conferir `limit 200` e `radar_hidden` no resultado.
- **`endodirect_acervo_totais()`** — contagens do acervo (provas, diretrizes, resumos, artigos, mapas, podcasts) **+ `temas` desde 25/09/2026** (`{sub:[tema,…]}`: capítulos privados publicados na ordem do array + temas extras do professor, sem repetição, só nomes). Embutida em `member_content` e `public_content` (chave `acervo_totais`, guardada em `lsGet('acervo_totais')`). Corpo completo em `supabase/temas-dos-resumos-no-acervo.sql`. ACL só `postgres`/`service_role` (chamada de dentro das RPCs).
- **`endodirect_acessos_ativos()`** (text[]) — desde 24/09 o **cliente a chama direto** (`carregarAcessos`, 3 tentativas, em paralelo com o conteúdo, cache por `uid` no aparelho): o plano do aluno deixa de depender da resposta grande. É também o que o poll do checkout consulta a cada 5 s (antes refazia o `member_content` inteiro).
