...
- **💬 O FEEDBACK DO APP CAI NESTA MESMA CAIXA (desde 03/08).** O modal de Feedback (⭐ + categoria + tipo + texto) manda pelo **mesmo** `kind:'support'`, com `category` = **`Feedback · <categoria>`** e a nota no corpo da mensagem (`⭐⭐⭐⭐ 4/5 · Flashcards · Problema`). O `context` diz se foi **espontâneo** (botão da topbar) ou **pedido no app** (convite automático a quem assina há mais de 30 dias — ver `endodirect_meu_feedback()` em [[Dados e Supabase]]) e há quantos dias a pessoa assina. Não roteia por subespecialidade (só `Dúvida` roteia), então **fica com o supervisor**. ⚠️ Até 03/08 esse modal **descartava** o que o aluno escrevia — ver [[Decisões]].
