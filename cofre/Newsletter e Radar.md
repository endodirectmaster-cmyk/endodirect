...
- **A Figura 1 (algoritmo de tratamento) FOI reproduzida** — corrigido no mesmo dia, a pedido do professor ("colocar a figura 1 original do artigo"). **⚠️ Eu havia escrito aqui que ela não seria reproduzida "mesmo sob CC BY", invocando a regra de 28/07 de referenciar figura pela legenda. A regra estava sendo aplicada fora do contexto em que nasceu:** ela veio de artigo **NC/ND**, onde a imagem realmente não pode ser copiada. Sob **CC BY 4.0 a imagem é tão redistribuível quanto a tabela** — e eu tinha acabado de escrever, dois parágrafos acima, que "acesso aberto não é um regime só". Apliquei ao texto o raciocínio que não apliquei à figura.
  - **03/08, depois: a tabela em markdown SAIU** ("pode retirar a tabela") — com a ficha logo acima dizendo o mesmo, ela era repetição atrás de um clique. **⚠️ Junto com a tabela teve de sair a atribuição dela.** O parágrafo acima dizia "**Tabela 1 …** *Reproduzida do artigo (European Thyroid Journal, licença CC BY 4.0)*"; sem a tabela, essa frase passaria a ficar imediatamente acima de um infográfico **do Endodirect** e o creditaria ao ETJ. Virou "**O sistema de escore, adaptado de Popoveniuc e cols. (7)**" + legenda "*Ficha Endodirect. Os pontos e os cortes são os do consenso (…CC BY 4.0)*": **o crédito acompanha o DADO, não a imagem**. A atribuição da **Figura 1** continua, porque aquela imagem é de fato reproduzida do artigo. 13.299 → 12.253 caracteres, 3 imagens, 5 seções, zero linha de tabela.
  - **03/08: entraram também DOIS infográficos do professor** — "Quando suspeitar: principais elementos clínicos" e "Escore diagnóstico de Popoveniuc", material próprio do Endodirect (`figuras/etj-26-0044-quando-suspeitar.jpg` e `…-escore-popoveniuc.jpg`). O primeiro abre a seção "Quando suspeitar"; o segundo entra entre a legenda da Tabela 1 e a tabela recolhida — **imagem para ler, tabela para copiar**, já que texto em imagem não é selecionável. ⚠️ **O escore do infográfico foi conferido item a item contra a Tabela 1 antes de publicar** (0/10/20 · 0/10/15/20/30 · 5/10/15 · 0/10 · 0/10/20/30 · 10/10/15/20 · 5×10, cortes ≥60 / 45–59 / <45): tudo bate. Material de marca com número clínico errado é pior que material nenhum, e a conferência custa dois minutos.
  - **⚠️ Publicar o ARQUIVO antes de referenciá-lo no markdown.** `muralInlineHTML` põe `onerror="this.remove()"` na tag: imagem que não carrega **não deixa ícone quebrado nem erro em log — ela some**. Gravar a referência antes do deploy deixaria a discussão sem a figura no intervalo, sem nada indicando por quê. O teste passou a cobrir os **três** arquivos por esse mesmo motivo.
