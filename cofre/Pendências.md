...
- [ ] **Memed — código PRONTO, falta chave (2026-06-11).** Integração 100% implementada e *gated*: backend `api/memed/token.js` (sincroniza prescritor + token do SDK) e front no Consultório (SDK Sinapse Prescrição, `setPaciente` com nome/CPF/e-mail/telefone/nascimento, captura do evento `prescricaoImpressa` -> prontuário `presc_emitidas`). Sem `MEMED_API_KEY`/`MEMED_SECRET`, usa o receituário próprio (fallback) — **não bloqueia o lançamento**. Pendente do usuário: (a) chave de **homologação** p/ a Memed validar; (b) preencher o formulário de Parceiros (WEBPKI = **Não**; URL `https://www.endodirect.com.br`; login de teste **memed.teste@endodirect.com.br** [senha fora do repo] já com Gold). Com a chave de produção, é só setar na Vercel — sem mexer no código. Ver [[Integrações]].
