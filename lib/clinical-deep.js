...
const TERMOS = {
  ...
  // ❌ `tetania`, `chvostek`, `trousseau` e `parestesia perioral` FICARAM DE FORA,
  ...
  // ❌ `pth` também ficou de fora: levaria "hipercalcemia com PTH inapropriadamente
  // normal" para uma área cujo único bloco de paratireoide é o de HIPOpara —
  // área certa, bloco errado. Entra junto do artigo de hiperparatireoidismo.
  ...
  'captacao': 'Tireoide', 'checkpoint': 'Tireoide',
  ...
  'inibidor de checkpoint': 'Tireoide', 'tirosina-quinase': 'Tireoide',
...
function deepFor(area, limite, tema) {
  ...
  const termos = [...vistos].filter((w) => (w.length >= 4 || conhecido(w)) && !VAZIAS.has(w) && w !== nomeArea);
  ...
  const bordas = (w) => {
    ...
    if (!re) { re = new RegExp("(^|[^a-z0-9])" + w.replace(/[.*+?^${}()|[\]\\]/g, "\\const pontuados = DEEP[canon].map((b, i) => {") + "(s|es)?([^a-z0-9]|$)"); _bordas.set(w, re); }
    return re;
  };
