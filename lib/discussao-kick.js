...
// Dispara uma invocação da cadeia. NÃO espera a resposta — ela demora ~40s e
// quem dispara não pode ficar preso nisso. Aborta a espera; o que importa é a
// invocação do outro lado ter começado, e ela roda até o fim por conta própria.
//
// ⚠️ O ABORT ERA DE 2s E ISSO É POUCO. Do outro lado pode haver partida a frio,
// e o lote inteiro sobe de uma vez — quem chega por último espera mais. Com 2s,
// um lote de 6 subiu 3 e os outros nunca foram invocados, sem erro nenhum.
// 6s custa 6s uma única vez (os disparos saem em paralelo).
const ESPERA_DISPARO_MS = 6000;

async function dispararCadeia(extra) {
  if (!process.env.CRON_SECRET) return false;
  const base = process.env.PUBLIC_BASE_URL || 'https://www.endodirect.com.br';
  const ctrl = new AbortController();
  const t = setTimeout(() => ctrl.abort(), ESPERA_DISPARO_MS);
  try {
