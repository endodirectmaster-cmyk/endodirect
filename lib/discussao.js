...
${temTab ? `- As TABELAS abaixo vêm do artigo. **Reproduza no corpo da discussão, em markdown**, as
  ...
  dizendo o que ela mostra; depois, o que se lê nela. Mantenha os valores EXATOS.` : '- O artigo não tem tabelas extraíveis.'}
...
// O aluno precisa saber de onde veio o texto e o que NÃO está ali. Sem esta
// linha, uma discussão longa passa a impressão de ter sido lida do PDF inteiro
// com as figuras à vista.
function rodape(ft) {
  const partes = [`texto integral (PMC ${ft.pmcid}, ${ft.palavras.toLocaleString('pt-BR')} palavras)`];
  if (ft.tabelas.length) partes.push(`${ft.tabelas.length} tabela(s) reproduzida(s) do artigo`);
  if (ft.figuras.length) partes.push(`${ft.figuras.length} figura(s) descrita(s) pela legenda, não reproduzidas`);
  return `\n\n---\n\n*Discussão elaborada sobre o ${partes.join('; ')}. Licença: ${ft.licenca.cc || 'não declarada'}. Para as imagens originais, consulte o artigo pelo link acima.*`;
}
