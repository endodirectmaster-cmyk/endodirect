...
// ⚠️ O CRITÉRIO É ESTRUTURAL, NÃO DE VOCABULÁRIO — e foi calibrado nas quatro
// tabelas desse tipo que já estavam gravadas, medindo colunas e linhas de cada:
//
...
//   5 col ×  5 lin (definições de PTx)               → FICA: compacta e legível
//
...
// vocabulário do autor, e a fronteira em 6 colunas cai exatamente entre a tabela
// compacta que vale a pena e as três que viram bloco de rolagem horizontal.
const MIN_COL_ESTUDOS = 6;
const MIN_LIN_ESTUDOS = 5;
function ehTabelaDeEstudosIncluidos(cabecalhos, nLinhasDeDados) {
  if (!RE_COL_ESTUDO.test(String(cabecalhos[0] || '').trim())) return false;
  return cabecalhos.length >= MIN_COL_ESTUDOS && nLinhasDeDados >= MIN_LIN_ESTUDOS;
}
...
function tableToMarkdown(tabelaXml) {
  ...
  if (ehTabelaDeEstudosIncluidos(corpoLinhas[0], corpoLinhas.length - 1)) return '';
