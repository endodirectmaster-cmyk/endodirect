...
// ⚠️ ESTA FUNÇÃO ESTAVA INVERTIDA NAS DUAS PONTAS (achado em 03/08).
//
// O teste era `/^CC (BY|0)/.test(cc)`, que ancora só o INÍCIO — e "CC BY-NC",
// "CC BY-NC-ND", "CC BY-ND" e "CC BY-SA" todas começam com "CC BY". As quatro
// saíam como **redistribuíveis**, exatamente as que a função existe para barrar.
// Na outra ponta, "CC0" não tem espaço, então `CC (BY|0)` não casava: o domínio
// público, que é o caso mais permissivo que existe, saía como **restrito**.
//
// O campo nunca tinha sido consumido por ninguém, então o erro estava dormente
// desde sempre. Ele só apareceu porque o gate de tabelas passou a lê-lo e eu
// testei a função em vez de reescrever a regra dela dentro do teste.
//
// ⚠️ A COMPARAÇÃO AGORA É EXATA, não por prefixo. Qualquer sufixo (NC, ND, SA)
// tira o direito: `NC` proíbe uso comercial e a plataforma é paga; `ND` proíbe
// derivado; `SA` obrigaria a plataforma inteira a herdar a licença.
function parseLicense(xml) {
  ...
  if (m) cc = 'CC ' + m[1].toUpperCase();
  else if (/creativecommons\.org\/publicdomain\/zero|public domain|\bcc0\b/.test(alvo)) cc = 'CC0';
  // Licença declarada só em prosa. ⚠️ Do mais restritivo para o mais permissivo:
  // "cc by-nc-nd" contém "cc by", e testar o curto primeiro classificaria a mais
  // restritiva de todas como a mais aberta — o mesmo erro do prefixo, por outro
  // caminho.
  else if (/\bcc[ -]?by[ -]?nc[ -]?nd\b/.test(alvo)) cc = 'CC BY-NC-ND';
  else if (/\bcc[ -]?by[ -]?nc[ -]?sa\b/.test(alvo)) cc = 'CC BY-NC-SA';
  else if (/\bcc[ -]?by[ -]?nc\b/.test(alvo)) cc = 'CC BY-NC';
  else if (/\bcc[ -]?by[ -]?nd\b/.test(alvo)) cc = 'CC BY-ND';
  else if (/\bcc[ -]?by[ -]?sa\b/.test(alvo)) cc = 'CC BY-SA';
  else if (/\bcc[ -]?by\b/.test(alvo)) cc = 'CC BY';
  return { url: href, texto, cc, redistribuivel: cc === 'CC BY' || cc === 'CC0' };
}
...
function fullTextForPrompt(ft, maxChars) {
  ...
  if (ft.tabelas.length) {
    // ⚠️ O MARCADOR SÓ EXISTE SE A LICENÇA PERMITIR REPRODUZIR.
    //
    // `parseLicense` calculava `redistribuivel` (só CC BY e CC0) desde sempre e
    // NINGUÉM o consumia — nem para figuras, ao contrário do que o cofre dizia.
    // Resultado: a tabela do JATS era colada palavra por palavra qualquer que
    // fosse a licença. A plataforma é paga (uso comercial, que `NC` proíbe) e
    // reformatar/traduzir tabela é derivado (que `ND` proíbe).
    //
    // ⚠️ O CONTEÚDO DA TABELA CONTINUA INDO PARA A IA nos dois casos, de
    // propósito: número é FATO, e fato não é protegido. O que a licença barra é
    // a REPRODUÇÃO — então, sem direito de reproduzir, a IA recebe os dados e
    // escreve os números em prosa, com redação própria. Foi o que fiz à mão no
    // artigo de hipercortisolismo (CC BY-NC-ND) em 31/07. Cortar a tabela do
    // prompt empobreceria a discussão sem necessidade nenhuma.
    var reproduzivel = !!(ft.licenca && ft.licenca.redistribuivel);
    anexos.push('## Tabelas do artigo\n'
      + (reproduzivel ? '' : '⚠️ A licença deste artigo (' + ((ft.licenca && ft.licenca.cc) || 'não declarada')
          + ') NÃO permite reproduzir a tabela. Use os números no texto, com redação própria,'
          + ' e NÃO escreva marcador de tabela.\n\n')
      + ft.tabelas
        .map((t, i) => (reproduzivel ? `[[TABELA:${i + 1}]] — ` : '')
          + `${t.rotulo}: ${t.legenda}\n${t.markdown}${t.nota ? '\n' + t.nota : ''}`)
        .join('\n\n'));
  }
