...
function fullTextForPrompt(ft, maxChars) {
  ...
  const partes = [];
  for (const s of ft.secoes) {
    partes.push('## ' + (s.titulo || 'Seção') + '\n' + s.paragrafos.join('\n\n'));
  }
  if (ft.figuras.length) {
    partes.push('## Figuras do artigo\n' + ft.figuras
      .map((f) => `${f.rotulo}: ${f.legenda}`).join('\n\n'));
  }
  if (ft.tabelas.length) {
    partes.push('## Tabelas do artigo\n' + ft.tabelas
      .map((t) => `${t.rotulo}: ${t.legenda}\n${t.markdown}${t.nota ? '\n' + t.nota : ''}`).join('\n\n'));
  }
  let out = partes.join('\n\n');
  if (out.length > teto) out = out.slice(0, teto).replace(/\s+\S*$/, '') + '\n\n[…texto truncado]';
  return out;
}
