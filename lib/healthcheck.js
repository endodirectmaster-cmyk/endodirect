  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f3f4f6;padding:24px 12px"><tr><td align="center">
    <table role="presentation" width="640" cellpadding="0" cellspacing="0" style="max-width:640px;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e5e7eb">
      <tr><td style="background:#1e3a5f;padding:20px 26px">
        <div style="color:#fff;font-size:19px;font-weight:800">Endodirect — Health check</div>
      ...
      <tr><td style="padding:6px 26px 20px">
        ...
        <div style="font-size:12px;color:#9ca3af;margin-top:14px">Verificação automática diária (resumo às segundas; alerta sempre que há falha). 🟢 ok · 🟡 atenção · 🔴 falha.</div>
      </td></tr>
    </table>
...
async function sendReport(checks, resumo, to) {
  ...
  const r = await fetch('https://api.resend.com/emails/batch', {
    ...
    body: JSON.stringify(to.map((dest) => ({ from, to: [dest], subject: `${worst} Endodirect — Health check`, html: reportHtml(checks, resumo) })))
  });
...
async function runHealthcheck() {
  ...
  if (!pk || !sk) add('Pagamentos (pagar.me)', 'fail', 'Chave pública/secreta do pagar.me ausente — checkout não processa.');
    ...
    // pagar.me v5: chaves de teste são pk_test_/sk_test_; as de produção são pk_/sk_ (sem "_live_").
    const isTest = /^pk_test_/.test(pk) || /^sk_test_/.test(sk);
    add('Pagamentos (pagar.me)', isTest ? 'warn' : 'ok', isTest ? 'Modo TESTE (pk_test) — pagamentos não são reais.' : 'Modo LIVE (vendas reais).');
  }
  ...
  // Roda diariamente (cron). Para virar ALERTA sem virar ruído: envia SEMPRE que
  // houver falha (qualquer dia); sem falha, só manda o resumo às segundas (UTC).
  // Sem falha e fora da segunda → silêncio (não envia nada).
  const isWeeklySummary = (new Date()).getUTCDay() === 1; // 1 = segunda-feira
  const shouldSend = fail > 0 || isWeeklySummary;
  let email = { sent: false, reason: shouldSend ? 'skipped' : 'no_issues' };
  let to = [];
  if (shouldSend) {
    to = recipients(await adminEmails(key));
  ...
  return { ok: fail === 0, resumo, checks, email, recipients: to.length, sent: shouldSend };
}
