...
function igTodayPlan(d) {
  ...
  if (dow === 2 || dow === 4 || dow === 6) return { kind: 'answer', dow }; // ter/qui/sáb → gabarito
  return { kind: 'question', dow }; // seg/qua/sex → questão
}
...
function questionCaption(it) {
  ...
  lines.push('');
  lines.push('👉 Responda no quiz acima! O gabarito sai amanhã.');
  lines.push('');
  lines.push('Estude endocrinologia de verdade no EndoDireto — link na bio. 📲');
  lines.push('#endocrinologia #residenciamedica #provadetitulo #endodireto #medicina');
  return lines.join('\n');
}
function answerCaption(it) {
  ...
  lines.push('Quer treinar com milhares de questões comentadas? EndoDireto — link na bio. 📲');
  lines.push('#endocrinologia #residenciamedica #endodireto #medicina');
  return lines.join('\n');
}
  ...
  return `<!doctype html><html><body style="margin:0;background:#f3f4f6;font-family:Arial,Helvetica,sans-serif;padding:24px 12px">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td align="center">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;background:#fff;border-radius:14px;overflow:hidden;border:1px solid #e5e7eb">
        <tr><td style="background:#0b1325;padding:20px 24px;color:#f5b32c;font-size:18px;font-weight:800">📲 Story do dia — EndoDireto</td></tr>
...
async function sendIgDailyNotice(now) {
  try {
    ...
    if (plan.kind === 'question') {
      ...
      if (it) {
        ...
        body = `<p style="margin:0;font-size:15px;color:#111827">Hoje (<b>${dia}</b>) é dia de <b>Questão do Dia</b> — subespecialidade <b>${esc(it.sub || 'Endocrinologia')}</b>.</p>
                <p style="margin:10px 0 0;font-size:14px;color:#374151">Abra o painel, baixe a arte (slide da pergunta) e poste o story às <b>18h</b> com o sticker de quiz (4 alternativas). O gabarito sai amanhã.</p>`;
      } else {
    ...
    const r = await fetch('https://api.resend.com/emails/batch', {
      ...
      body: JSON.stringify(to.map((dest) => ({ from, to: [dest], subject: `EndoDireto — ${subject}`, html })))
    });
