...
// É a newsletter "dos artigos DO DIA", então a ordem é:
// 1º) FRESCOR (faixa de dias desde a publicação) — o destaque tem que ser recente;
// 2º) tipo: Revisão/diretriz/consenso > Metanálise/revisão sistemática > Original/ensaio;
// 3º) revista: NEJM → Lancet → JCEM → demais;
// 4º) data exata (mais novo primeiro).
// Antes a ordem começava pelo TIPO, então uma diretriz de semanas atrás (alto tier)
// liderava o e-mail mesmo havendo artigos novos — saía "desatualizada".
// Prioridade por TIPO (2º critério): Revisão/artigo de revisão (+ diretriz/consenso) >
// Metanálise/revisão sistemática > Artigo original/ensaio clínico > demais.
function articleTypeTier(tipo) {
  const t = String(tipo || '').toLowerCase();
  if (t.indexOf('metanal') >= 0 || t.indexOf('meta-anal') >= 0 || t.indexOf('sistemat') >= 0) return 1; // metanálise/revisão sistemática
  if (t.indexOf('revis') >= 0 || t.indexOf('diretriz') >= 0 || t.indexOf('consenso') >= 0) return 0;     // artigo de revisão / guidance
  if (t.indexOf('ensaio') >= 0 || t.indexOf('original') >= 0 || t.indexOf('coorte') >= 0 || t.indexOf('caso-controle') >= 0 || t.indexOf('clinical trial') >= 0) return 2; // original/ensaio
  return 3; // demais
}
// Desempate por PERIÓDICO: NEJM > Lancet > JCEM > demais.
function journalRank(fonte) {
  ...
  if (f.indexOf('j clin endocrinol') >= 0 || f.indexOf('clinical endocrinology') >= 0 || f.indexOf('jcem') >= 0) return 2;
  return 3; // demais revistas de impacto
}
...
function rankArticles(items) {
  return (Array.isArray(items) ? items : []).map((a, i) => ({ a, i })).sort((x, y) => {
    ...
    if (fa !== fb) return fa - fb;                 // 1º) mais recente (por faixa de dias)
    const ta = articleTypeTier(x.a.tipo), tb = articleTypeTier(y.a.tipo);
    if (ta !== tb) return ta - tb;                 // 2º) revisão/diretriz > metanálise > original
    const ja = journalRank(x.a.fonte), jb = journalRank(y.a.fonte);
    if (ja !== jb) return ja - jb;                 // 3º) NEJM → Lancet → JCEM → demais
