...
function formatSources(arts) {
  ...
  const lines = arts.map(function (a) {
    ...
    return '- [' + (a.year || 's/d') + '] ' + a.title + ' (' + a.journal + ')' + ab;
  });
  return 'FONTES (PubMed, recentes e verificadas — use para EMBASAR a resposta; ao citar, refira-se pela DIRETRIZ/SOCIEDADE + ano ou pelo periódico + ano, SEM exibir PMID, DOI ou links; se nenhuma servir, baseie-se nas diretrizes nomeadas; NUNCA invente referências):\n' + lines.join('\n');
}
