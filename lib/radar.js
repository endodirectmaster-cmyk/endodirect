...
async function buildMuralItem(article, apiKey) {
  ...
  const dataLinha = article.publicationDate ? `📅 Data de publicacao: ${formatPubDateBR(article.publicationDate)}\n` : '';
  const texto =
`${dataLinha}🔬 Tipo de estudo: ${tipoEstudo}
📝 Resumo: ${resumo}
💡 Por que importa na pratica: ${porque}
⚠️ Cautela/limitacao: ${cautela}`;
  return {
    titulo: article.title,
    tipo: tipoMural,
...
const SOCIETY_SOURCES = [
  {
    ...
    feeds: [
      { url: 'https://www.endocrino.org.br/categoria_noticias/comunicados-oficiais/feed/', tipo: 'Diretriz' }
    ]
  }
];
...
function parseRssItems(xml) {
  ...
  while ((m = re.exec(xml)) && items.length < 40) {
    ...
    if (!/^https?:\/\//i.test(link)) { const g = stripHtml(rssTag(b, 'guid')); if (/^https?:\/\//i.test(g)) link = g; }
    const pubDate = stripHtml(rssTag(b, 'pubDate') || rssTag(b, 'dc:date'));
    const desc = rssTag(b, 'content:encoded') || rssTag(b, 'description');
...
Responda SOMENTE com JSON neste formato exato:
{"subespecialidade":"<exatamente UM de: ${SUB_NAMES.join(' | ')}>","tipoMural":"<exatamente UM de: ${MURAL_TYPE_NAMES.join(' | ')}>","resumo":"<3 a 5 linhas com o que o comunicado diz de mais importante, em portugues claro>","porque":"<1-2 frases: por que importa na pratica clinica no Brasil>"}
Regras: use apenas o conteudo fornecido; nao invente numeros nem afirmacoes; portugues do Brasil. Se for um posicionamento/nota oficial ou diretriz, use tipoMural "Diretriz".`;
  try {
    const r = await fetch('https://api.anthropic.com/v1/messages', {
  ...
  for (const src of SOCIETY_SOURCES) {
    for (const feed of src.feeds) {
      ...
      for (const e of entries) {
        ...
        const resumo = (ai && ai.resumo) || String(e.summary || '').slice(0, 600);
        if (!resumo) continue;
        const porque = (ai && ai.porque) || `Posicionamento/comunicado oficial da ${src.sigla} — orienta a conduta e a comunicacao com pacientes no Brasil.`;
        const subEsp = (ai && matchCanonical(ai.subespecialidade, SUB_NAMES)) || '';
        const tipoMural = (ai && matchCanonical(ai.tipoMural, MURAL_TYPE_NAMES)) || feed.tipo || 'Diretriz';
        const dataBR = Number.isFinite(t) ? new Date(t).toLocaleDateString('pt-BR') : '';
        const texto =
`${dataBR ? '📅 Publicado em: ' + dataBR + '\n' : ''}🏛️ Fonte: ${src.sigla} (${src.nome})
📝 Resumo: ${resumo}
💡 Por que importa na pratica: ${porque}`;
        out.push({
          titulo: e.title,
...
async function runRadar() {
  ...
  const allIncoming = breaking.concat(society, incomingClean); // breaking + sociedades primeiro (topo)
