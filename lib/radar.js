...
async function findRelevantArticles(excludeKeys) {
  ...
  for (const sub of SUBSPECIALTIES) {
    const arts = (idsBySub[sub.nome] || [])
      ...
      .filter((a) => a.abstract && a.abstract.trim().length >= 80) // exige abstract (não inclui artigos sem resumo)
...
async function runRadar() {
  ...
  const allIncoming = breaking.concat(incoming); // breaking primeiro (aparece no topo)
