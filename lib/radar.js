...
const { runNews, saudeDosFeeds, isBreakingTrusted } = require('./news');
...
function mergeMuralItems(payload, incoming) {
  ...
  // ⚠️ O que o professor APAGOU do mural (03/08/2026). existingMuralKeys ja
  // impede a RE-entrada de um item oculto, mas o ramo `retained` abaixo mantinha
  // vivo o que ja estava gravado — e o cron escreve como service_role, sem passar
  // pelo gatilho que aplica radar_hidden. Sem este filtro, um item apagado
  // sobrevivia no payload ate o TTL de 90 dias e seguia sendo entregue ao aluno.
  const ocultos = new Set((Array.isArray(payload.radar_hidden) ? payload.radar_hidden : []).map(String).filter(Boolean));
  const fresh = incoming.filter((item) => !existingKeys.has(item.sourceId) && !existingKeys.has(item.link) && !existingKeys.has(item.titulo))
    // Segunda trava: `existingMuralKeys` ja exclui o apagado da BUSCA, mas se um
    // item oculto chegar por outro caminho ele nao pode entrar por aqui.
    .filter((item) => !ocultos.has(String(keyOf(item) || '')));
  const retained = current.filter((item) => {
    const itemTime = Number(item.at) || 0;
    if (now - itemTime >= AUTO_ITEM_TTL_MS) return false;
    if (ocultos.has(String(keyOf(item) || ''))) return false;
...
async function runRadar() {
  ...
  // ⚠️ Feed oficial que falha ou volta VAZIO sai daqui para o alerta do cron. Um
  // feed morto não derruba o radar (é `fail-safe` por desenho) e, sem isto, também
  // não avisa: o da Lilly ficou mudo o acervo inteiro sem ninguém notar. Só os
  // OFICIAIS entram no alerta — feed de busca voltar vazio num dia é normal.
  let feedsMudos = [];
  try { feedsMudos = saudeDosFeeds().filter((f) => f.official && (!f.ok || f.itens === 0)); }
  catch (e) { feedsMudos = []; }
