...
const JOURNALS = [
  ...
  { name: 'Fertility and Sterility', query: 'Fertil Steril', weight: 7 },
...
📝 Resumo: ${resumo}
💡 Por que importa na prática: ${porque}
...
const SOCIETY_SOURCES = [
  ...
  {
    // American Thyroid Association (2026-07-20, "puxar mais notícias/revistas").
    // thyroid.org é WordPress → feed padrão em /feed/. Best-effort: se não
    // responder, é ignorado (nunca derruba o radar). Título em inglês; a IA
    // resume em PT como os demais.
    sigla: 'ATA',
    nome: 'American Thyroid Association',
    feeds: [
      { url: 'https://www.thyroid.org/feed/' }
    ]
  },
  {
    // Endocrine Society — publicação "Endocrine News" (endocrinenews.endocrine.org,
    // WordPress → /feed/). Notícias/atualizações oficiais da sociedade. Best-effort;
    // se o feed não responder, é ignorado. Verificar no 1º run do radar quais URLs
    // realmente respondem (não foi possível validar externamente do ambiente do build).
    sigla: 'Endocrine Society',
    nome: 'Endocrine Society (Endocrine News)',
    feeds: [
      { url: 'https://endocrinenews.endocrine.org/feed/' }
    ]
  }
];
...
const JOURNAL_RSS_SOURCES = [
  ...
  { sigla: 'Bone', journal: 'Bone', feeds: [{ url: 'https://rss.sciencedirect.com/publication/science/87563282' }] },
  { sigla: 'FertilSteril', journal: 'Fertility and Sterility', feeds: [{ url: 'https://rss.sciencedirect.com/publication/science/00150282' }] },
  { sigla: 'DOM', journal: 'Diabetes, Obesity and Metabolism', feeds: [{ url: 'https://dom-pubs.onlinelibrary.wiley.com/feed/14631326/most-recent' }] }
];
...
async function buildJournalRssItem(entry, src, apiKey) {
  ...
  const texto =
`${dataLinha}🔬 Tipo de estudo: ${tipoEstudo}
📝 Resumo: ${resumo}${porque ? `\n💡 Por que importa na prática: ${porque}` : ''}${cautela ? `\n⚠️ Cautela/limitação: ${cautela}` : ''}`;
  return {
    titulo: entry.title,
    tipo: tipoMural,
    fonte: src.journal,
...
async function fetchJournalRssItems(excludeKeys, apiKey) {
  ...
  for (const src of JOURNAL_RSS_SOURCES) {
    for (const feed of src.feeds) {
      ...
      for (const e of entries) {
        if (!e.title || !/^https?:\/\//i.test(e.link || '')) continue;
        if (excludeKeys.has(e.link) || excludeKeys.has(e.title) || excludeKeys.has(`journalrss:${src.sigla}:${e.link}`)) continue;
        const t = Date.parse(e.pubDate || '');
