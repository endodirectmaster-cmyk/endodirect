...
async function findRelevantArticles(excludeKeys) {
  ...
  try { console.log('[radar-dbg] allIds=' + allIds.size + ' (IDs brutos das buscas por subespecialidade, antes da exclusão)'); } catch (e) {}
...
{"semAbstract":<true|false>,"tipo":"<tipo de estudo em portugues>","resumo":"<3 a 5 linhas: objetivo, metodo e principal achado, em portugues claro>","porque":"<1-2 frases: por que importa na pratica clinica>","cautela":"<1 frase de cautela/limitacao metodologica>"}
...
async function runRadar() {
  ...
  const allIncoming = breaking.concat(incomingClean); // breaking primeiro (aparece no topo)
  ...
  // Diagnóstico (temporário): mostra onde o pipeline para — quantos artigos o
  // PubMed devolveu, quantos a IA resumiu, breaking, quantos ficaram "frescos"
  // (não-duplicados) e o total final. Ajuda a ver por que o Mural não atualiza.
  try {
    console.log('[radar-dbg] pubmed=' + (Array.isArray(articles) ? articles.length : -1)
      + ' ia_ok=' + incomingClean.length + ' breaking=' + breaking.length
      + ' fresh=' + merged.fresh.length
      + ' total=' + ((merged.payload && Array.isArray(merged.payload.radar_avisos)) ? merged.payload.radar_avisos.length : -1)
      + ' anthropic=' + (anthropicKey ? 'set' : 'MISSING')
      + ' exclude=' + (excludeKeys && excludeKeys.size != null ? excludeKeys.size : (Array.isArray(excludeKeys) ? excludeKeys.length : '?')));
  } catch (e) {}
