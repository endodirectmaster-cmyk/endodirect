...
// Roteamento das DÚVIDAS por subespecialidade → e-mail do professor responsável.
// ⚠️ Manter em sincronia com QOTD_CURATORS no index.html (mesma distribuição).
const SUPPORT_ROUTING = {
  'Obesidade': 'rodolphomend@gmail.com',
  'Lípides': 'rodolphomend@gmail.com',
  'Neuroendocrinologia': 'rodolphomend@gmail.com',
  'Adrenal': 'drrafaelgiorgi@hotmail.com',
  'Endocrinologia do Esporte': 'drrafaelgiorgi@hotmail.com',
  'Endocrinologia Feminina': 'drrafaelgiorgi@hotmail.com',
  'Endocrinologia Masculina': 'drrafaelgiorgi@hotmail.com',
  'Diabetes': 'nakamura.epm79@gmail.com',
  'Tireoide': 'nakamura.epm79@gmail.com',
  'Osteometabolismo': 'nakamura.epm79@gmail.com',
  'Transgeneridade': 'nakamura.epm79@gmail.com',
  'Endocrinologia Pediátrica': 'brunosimiao1906@gmail.com'
};
// Supervisores veem TODAS as mensagens (Rodolpho). Os demais professores só as suas.
const SUPPORT_SUPERVISORS = ['rodolphomend@gmail.com', 'endodirectmaster@gmail.com'];
// Só roteia DÚVIDA com subespecialidade conhecida; o resto fica geral (só supervisor).
function routeAssignee(category, subspecialty) {
  const c = String(category == null ? '' : category).trim().toLowerCase();
  if (c !== 'dúvida' && c !== 'duvida') return '';
  return SUPPORT_ROUTING[String(subspecialty == null ? '' : subspecialty).trim()] || '';
}

// Salva o ticket recebido do formulário. Best-effort: se a tabela/chave faltar,
// NÃO derruba o envio do e-mail de notificação. Retorna { stored, id, assigned_email, ... }.
async function storeSupportTicket(data) {
  try {
    data = data || {};
    // E-mail canônico em minúsculas: é a CHAVE que liga o ticket ao aluno quando
    // ele acompanha as respostas no app (listMyTickets casa pelo e-mail da sessão).
    ...
    const subspecialty = clean(data.subspecialty, 80) || null;
    const assigned_email = routeAssignee(data.category, data.subspecialty) || null;
    const row = {
    ...
    return { stored: true, id: (rows && rows[0] && rows[0].id) || null, assigned_email, subspecialty, category: clean(data.category, 60), name: clean(data.name, 120) };
  } catch (e) {
...
async function listSupportTickets(adminEmail) {
  try {
    const key = serviceKey();
    if (!key) return [];
    const me = String(adminEmail == null ? '' : adminEmail).trim().toLowerCase();
    // Supervisor (Rodolpho) vê tudo; cada professor vê só as dúvidas roteadas p/ ele.
    let filter = '';
    if (me && SUPPORT_SUPERVISORS.indexOf(me) < 0) filter = `&assigned_email=eq.${encodeURIComponent(me)}`;
    const r = await fetch(`${SUPABASE_URL}/rest/v1/endodirect_support?select=*&order=created_at.desc&limit=500${filter}`, { headers: serviceHeaders(key) });
...
// Lista os tickets de UM aluno (para ele acompanhar as respostas DENTRO do app).
// O servidor passa o `email` JÁ resolvido do token de sessão — NUNCA confiar em
// e-mail vindo do corpo (senão qualquer um leria a conversa alheia). Retorna só
// campos seguros: SEM `answered_by` (e-mail interno do professor). Novos primeiro.
async function listMyTickets(email) {
  try {
    const e = clean(email, 160).toLowerCase();
    if (!validEmail(e)) return [];
    const key = serviceKey();
    if (!key) return [];
    const cols = 'id,created_at,category,message,status,reply,answered_at';
    const r = await fetch(`${SUPABASE_URL}/rest/v1/endodirect_support?email=eq.${encodeURIComponent(e)}&select=${cols}&order=created_at.desc&limit=100`, { headers: serviceHeaders(key) });
    if (!r.ok) { console.error('[support] listMyTickets HTTP', r.status); return []; }
    const rows = await r.json().catch(() => []);
    return Array.isArray(rows) ? rows : [];
  } catch (err) {
    console.error('[support] listMyTickets falha:', (err && err.message) || err);
    return [];
  }
}
...
module.exports = { sendSupportEmail, storeSupportTicket, listSupportTickets, listMyTickets, replySupportTicket };
