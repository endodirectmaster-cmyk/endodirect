...
async function sendWinbackNovidades() {
  ...
  const led = (toSave.trial_emails && typeof toSave.trial_emails === 'object') ? toSave.trial_emails : {};
  const hoje = todayISO();
  // (a gravação abaixo usa `chave`, para cada nível ter a sua própria memória)
...
// ⚠️ NÍVEL CEDO, criado em 19/08/2026 depois de medir o funil. O toque de 14
// dias dispara certo e é inócuo: de 20 pessoas que o receberam, 1 voltou a
// estudar (5%). O motivo é o prazo — a perda acontece entre o DIA 1 e o DIA 2,
// então o e-mail chega duas semanas depois da desistência.
// Chave de ledger PRÓPRIA (`reengajamento_cedo`), senão os dois níveis dividem o
// cooldown de 30 dias e um cala o outro sem erro nenhum.
const REENG_CEDO_DIAS = 3;
const REENG_CEDO_COOLDOWN = 21;
const REENG_CEDO_CHAVE = 'reengajamento_cedo';
...
// Corpo do toque CEDO (3 dias). Não é "olha o que entrou" — é UMA questão, a
// mesma que está publicada na plataforma. O objetivo é ser respondido, não lido:
// a alternativa leva ao app, e responder lá conta como estudo (studyEvent).
//
// ⚠️ Sem gabarito no e-mail, de propósito. Se a resposta viesse aqui, o aluno
// resolveria mentalmente e não abriria o app — e é a abertura que tira ele do
// balde "nunca estudou".
function cedoHtml(payload, nome, emailTo) {
  const fila = Array.isArray(payload.ig_stories) ? payload.ig_stories : [];
  const postadas = fila.filter((x) => x && x.status === 'posted' && x.stem && x.options && x.answer);
  const q = postadas.length ? postadas[postadas.length - 1] : null;
  const cta = 'https://www.endodirect.com.br/';
  const ola = nome ? `Olá, ${esc(String(nome).split(' ')[0])}.` : 'Olá.';
  const alts = q
    ? ['A', 'B', 'C', 'D'].filter((L) => q.options[L]).map((L) =>
        `<tr><td style="padding:7px 10px;border:1px solid #e4e8f0;border-radius:8px;font-size:14px;color:#39435a;line-height:1.5">`
        + `<b style="color:#2c5fc4">${L}</b> &nbsp;${esc(q.options[L])}</td></tr><tr><td style="height:6px"></td></tr>`).join('')
    : '';
  return shell(
    `<p style="font-size:15px;color:#13294b;margin:0 0 10px">${ola}</p>`
    + `<p style="font-size:14px;color:#39435a;line-height:1.7;margin:0 0 16px">`
    + `Separei uma questão para você — leva menos de um minuto.</p>`
    + (q
      ? `<div style="background:#f8f9fc;border:1px solid #e4e8f0;border-radius:12px;padding:14px 16px;margin:0 0 18px">`
        + `<p style="font-size:11px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#8a93a6;margin:0 0 8px">Questão do dia`
        + (q.sub ? ` · ${esc(q.sub)}` : '') + `</p>`
        + `<p style="font-size:14px;color:#13294b;line-height:1.65;margin:0 0 12px">${esc(String(q.stem).slice(0, 460))}</p>`
        + `<table role="presentation" cellpadding="0" cellspacing="0" width="100%">${alts}</table>`
        + `</div>`
      : '')
    + btn(cta, q ? 'Responder no app' : 'Abrir a plataforma')
    + `<p style="font-size:13px;color:#5a6a8a;line-height:1.65;margin:18px 0 0">`
    + `Ao responder, o app começa a acompanhar seu desempenho por área e guarda o que você errar para revisão.</p>`
    + `<p style="font-size:12px;color:#8a93a6;margin:16px 0 0">Seu acesso está ativo — é só entrar.`
    + (emailTo ? ` Se preferir não receber estes avisos, <a href="${esc(unsubUrl(emailTo))}" style="color:#8a93a6">descadastre-se aqui</a>.` : '')
    + `</p>`
  );
}
...
// ═══ AVISO DE RENOVAÇÃO ════════════════════════════════════════════════════
// MEDIDO EM 19/08/2026: 31 das 36 assinaturas ativas são ANUAIS AVULSAS — não
// recorrem, não cobram sozinhas, exigem DECISÃO ATIVA de pagar de novo, e vencem
// entre 17/06 e 16/08/2027. Até aqui venciam EM SILÊNCIO: o único e-mail de
// vencimento que existia era o da DEGUSTAÇÃO (`warnHtml`).
//
// ⚠️ O RECORRENTE NÃO RECEBE ISTO (a RPC o exclui): ele cobra sozinho, e avisar
// quem não precisa decidir nada só ensina o assinante a ignorar o remetente.
const RENOV_MARCOS = [
  { janela: 30, min: 8 },   // um mês antes: tempo de decidir
  { janela: 7,  min: 2 },   // uma semana antes
  { janela: 1,  min: 0 },   // véspera
];
// Data ISO -> dd/mm/aaaa. A RPC devolve 'YYYY-MM-DD' e o e-mail é em português:
// "vence em 2027-06-23" dentro de uma frase em português lê-se como defeito.
function dataBR(iso) {
  const m = String(iso || '').match(/^(\d{4})-(\d{2})-(\d{2})/);
  return m ? `${m[3]}/${m[2]}/${m[1]}` : String(iso || '');
}

// ═══ "SEU ANO NO ENDODIRECT" ═══════════════════════════════════════════════
// O painel que vai DENTRO do aviso de renovação. Quem decide pagar de novo
// decide olhando para o que já tirou dali — e até aqui o e-mail de vencimento
// não existia, quanto mais com número.
//
// ⚠️ UM SÓ RECORTE DE TEMPO, e ele é a conta inteira ("desde que você entrou").
// Misturar "dias no período pago" com "acerto geral" produz números que não
// fecham entre si, e o assinante percebe.
//
// ⚠️ SÓ MOSTRA O QUE EXISTE. Célula com zero ("0 questões", "0% de acerto") em
// e-mail de renovação é argumento CONTRA renovar, escrito por nós mesmos.
function retrospectivaHtml(alvo) {
  const dias = Math.max(0, Number(alvo.dias_estudados) || 0);
  const resp = Math.max(0, Number(alvo.respondidas) || 0);
  const acertos = Math.max(0, Number(alvo.acertos) || 0);
  if (!dias && !resp) return '';
  const pct = resp ? Math.round((acertos / resp) * 100) : null;
  const celulas = [];
  if (dias) celulas.push([String(dias), dias === 1 ? 'dia de estudo' : 'dias de estudo']);
  if (resp) celulas.push([String(resp), resp === 1 ? 'questão respondida' : 'questões respondidas']);
  if (pct != null && resp >= 5) celulas.push([pct + '%', 'de acerto']);
  const largura = Math.round(100 / celulas.length);
  const numeros = `<table role="presentation" cellpadding="0" cellspacing="0" width="100%"><tr>`
    + celulas.map(([n, r]) => `<td width="${largura}%" style="text-align:center;padding:6px 4px">`
      + `<div style="font-size:26px;font-weight:800;color:#13294b;line-height:1.2">${esc(n)}</div>`
      + `<div style="font-size:12px;color:#8a93a6;line-height:1.4">${esc(r)}</div></td>`).join('')
    + `</tr></table>`;
  const desde = alvo.primeiro_dia ? ` Desde <b>${esc(dataBR(alvo.primeiro_dia))}</b>:` : '';
  const area = alvo.area_top
    ? `<p style="font-size:13px;color:#5a6a8a;line-height:1.6;margin:10px 0 0">A área que você mais estudou foi <b>${esc(String(alvo.area_top))}</b>.</p>`
    : '';
  return `<div style="background:#f8f9fc;border:1px solid #e4e8f0;border-radius:12px;padding:16px 18px;margin:18px 0">`
    + `<p style="font-size:11px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#8a93a6;margin:0 0 4px">Seu ano no Endodirect</p>`
    + `<p style="font-size:13px;color:#5a6a8a;line-height:1.6;margin:0 0 8px">O que você fez por aqui.${desde}</p>`
    + numeros + area + `</div>`;
}

function renovacaoHtml(alvo, dados) {
  const nome = alvo.nome ? String(alvo.nome).split(' ')[0] : '';
  const ola = nome ? `Olá, ${esc(nome)}.` : 'Olá.';
  const d = Math.max(0, Number(alvo.dias_para_vencer) || 0);
  const quando = d === 0 ? 'hoje' : (d === 1 ? 'amanhã' : `em ${d} dias`);
  const cta = publicBase() + '/#planos';
  const lista = (arr, titulo) => (arr && arr.length)
    ? `<p style="font-size:15px;color:#13294b;font-weight:700;margin:18px 0 8px">${titulo}</p>`
      + `<ul style="font-size:14px;color:#39435a;line-height:1.7;margin:0 0 6px;padding-left:20px">`
      + arr.map((it) => `<li>${esc(it.titulo)}</li>`).join('') + `</ul>`
    : '';
  // ⚠️ Para quem NUNCA usou, um resumo de "o que você fez" seria vazio e soaria
  // como cobrança. A esses mostra-se o que a plataforma PRODUZIU no período —
  // que é o que ele pagou para ter, tendo aberto ou não.
  //
  // ⚠️ E "usou" NÃO é só `act`. Sete contas respondem questões e têm `act` vazio:
  // responderam entre 25/06 e 05/07/2026, antes de `studyEvent` existir (25/07).
  // Olhando só para os dias, o e-mail diria "você não estudou" a quem respondeu
  // 41 questões.
  const usou = Number(alvo.dias_estudados) > 0 || Number(alvo.respondidas) > 0;
  return shell(
    `<h1 style="font-size:21px;color:#13294b;margin:0 0 10px">Seu acesso ao Endodirect vence ${quando}</h1>`
    + `<p style="font-size:15px;color:#13294b;margin:0 0 12px">${ola}</p>`
    + `<p style="font-size:14px;color:#39435a;line-height:1.7;margin:0 0 8px">`
    + `Sua assinatura <b>anual</b> termina em <b>${esc(dataBR(alvo.vence_em))}</b>. `
    + `Ela <b>não renova sozinha</b> — se quiser continuar, é preciso renovar.</p>`
    + (usou ? retrospectivaHtml(alvo) : '')
    + lista(dados && dados.diretrizes, 'Diretrizes e consensos que entraram no seu período')
    + lista(dados && dados.revisoes, 'Revisões que entraram')
    + btn(cta, 'Renovar meu acesso')
    // ⚠️ ITEM 2 DA ESTRATÉGIA, e o lugar certo dele é aqui: a hora de oferecer
    // recorrência é quando a pessoa está decidindo renovar. Hoje são 5
    // recorrentes contra 31 avulsas — inverter isso retém mais que qualquer
    // funcionalidade nova, porque o padrão passa a ser CONTINUAR.
    + `<div style="background:#f8f9fc;border:1px solid #e4e8f0;border-radius:12px;padding:14px 16px;margin:20px 0 0">`
    + `<p style="font-size:14px;color:#13294b;font-weight:700;margin:0 0 6px">Quer não precisar lembrar disso de novo?</p>`
    + `<p style="font-size:13px;color:#5a6a8a;line-height:1.65;margin:0">`
    + `Na renovação você pode escolher o <b>plano mensal recorrente</b>: a cobrança segue sozinha e você cancela quando quiser, `
    + `sem correr o risco de perder o acesso por esquecimento.</p></div>`
    + `<p style="font-size:12px;color:#8a93a6;margin:18px 0 0">`
    + (alvo.email ? `Se preferir não receber estes avisos, <a href="${esc(unsubUrl(alvo.email))}" style="color:#8a93a6">descadastre-se aqui</a>.` : '')
    + `</p>`
  );
}

// Dispara os avisos de renovação nos três marcos. Fail-safe: nunca lança.
async function sendRenovacao() {
  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey) { console.log('[renovacao] RESEND_API_KEY ausente — pulado.'); return { sent: false, reason: 'no_api_key' }; }
  const key = serviceKey();
  if (!key) { console.log('[renovacao] service key ausente — pulado.'); return { sent: false, reason: 'no_service_key' }; }
  const from = process.env.TRIAL_FROM || process.env.NEWSLETTER_FROM || 'Endodirect <newsletter@endodirect.com.br>';
  let total = 0; const porMarco = {};
  for (const marco of RENOV_MARCOS) {
    let alvos = [];
    try {
      const r = await fetch(`${SUPABASE_URL}/rest/v1/rpc/endodirect_renovacao_alvos`, {
        method: 'POST', headers: serviceHeaders(key),
        body: JSON.stringify({ p_janela: marco.janela, p_min: marco.min })
      });
      if (!r.ok) { console.error('[renovacao] RPC HTTP', r.status, 'marco', marco.janela); continue; }
      alvos = await r.json().catch(() => []);
    } catch (e) { console.error('[renovacao] RPC erro:', e && e.message); continue; }
    if (!Array.isArray(alvos) || !alvos.length) { porMarco[marco.janela] = 0; continue; }

    const payload = await loadPayload(key);
    const unsub = new Set((Array.isArray(payload.newsletter_unsub) ? payload.newsletter_unsub : []).map((e) => String(e).toLowerCase()));
    const validos = alvos.filter((a) => a && a.email && !unsub.has(String(a.email).toLowerCase())).slice(0, MAX_PER_RUN);
    if (!validos.length) { porMarco[marco.janela] = 0; continue; }

    // O que entrou no período que ele pagou — é isto que justifica a renovação.
    const dados = {
      diretrizes: novidadesDoMural(payload, ['Diretriz', 'Consenso'], 5),
      revisoes: novidadesDoMural(payload, ['Artigo de Revisão'], 5),
    };
    const porEmail = {}; validos.forEach((a) => { porEmail[String(a.email).toLowerCase()] = a; });
    const assunto = marco.janela === 1
      ? 'Seu acesso ao Endodirect vence amanhã'
      : `Seu acesso ao Endodirect vence em ${marco.janela} dias`;
    const enviados = await sendBatch(apiKey, from, assunto, validos.map((a) => a.email),
      (to) => renovacaoHtml(porEmail[String(to).toLowerCase()] || {}, dados));

    let toSave = payload;
    try { toSave = await loadPayload(key); } catch (e) { toSave = payload; }
    const led = (toSave.trial_emails && typeof toSave.trial_emails === 'object') ? toSave.trial_emails : {};
    const hoje = todayISO();
    // ⚠️ A chave vem da RPC e carrega a DATA DE VENCIMENTO: sem ela, quem
    // renovasse nunca mais receberia aviso — 'renov_30' ficaria marcado para
    // sempre e o ciclo seguinte passaria em branco.
    validos.forEach((a) => { led[String(a.email).toLowerCase()] = Object.assign({}, led[String(a.email).toLowerCase()], { [a.chave]: hoje }); });
    toSave.trial_emails = led;
    try { await savePayload(key, toSave); } catch (e) { console.error('[renovacao] falha ao gravar ledger:', e && e.message); }
    porMarco[marco.janela] = enviados;
    total += enviados;
  }
  console.log('[renovacao] enviados por marco:', JSON.stringify(porMarco));
  return { sent: true, enviados: total, porMarco };
}

// Dispara o reengajamento. Fail-safe: qualquer pré-condição ausente => pula.
async function sendReengajamento(opts) {
  const dias = (opts && opts.dias) || REENG_DIAS;
  const cooldown = (opts && opts.cooldown) || REENG_COOLDOWN;
  const chave = (opts && opts.chave) || 'reengajamento';
  const cedo = chave === REENG_CEDO_CHAVE;
  const apiKey = process.env.RESEND_API_KEY;
  ...
  try {
    const r = await fetch(`${SUPABASE_URL}/rest/v1/rpc/endodirect_reengajamento_alvos`, {
      method: 'POST', headers: serviceHeaders(key),
      body: JSON.stringify({ p_dias: dias, p_cooldown: cooldown, p_chave: chave })
    });
  ...
  // ⚠️ O gate de "sem novidade, não manda" vale para o toque de 14 dias, que é
  // um "olha o que entrou". O toque CEDO é outra coisa: leva UMA QUESTÃO. Ele
  // não depende de o mural ter novidade, e travá-lo aqui o mataria em silêncio.
  if (!cedo && !revisoes.length && !diretrizes.length) {
  ...
  const from = process.env.TRIAL_FROM || process.env.NEWSLETTER_FROM || 'Endodirect <newsletter@endodirect.com.br>';
  const assunto = cedo
    ? 'Uma questão de endocrinologia para hoje'
    : 'O que entrou no Endodirect enquanto você esteve fora';
  const enviados = await sendBatch(apiKey, from, assunto, lista,
    (to) => (cedo ? cedoHtml(payload, nomePorEmail[to], to) : reengajamentoHtml(dados, nomePorEmail[to], to)));
  ...
  const hoje = todayISO();
  // (a gravação abaixo usa `chave`, para cada nível ter a sua própria memória)
  lista.forEach((e) => { led[e] = Object.assign({}, led[e], { [chave]: hoje }); });
  toSave.trial_emails = led;
...
module.exports = {
  ...
  sendRenovacao, renovacaoHtml, retrospectivaHtml, dataBR, RENOV_MARCOS, cedoHtml,
  CAMPANHA, REENG_DIAS, REENG_COOLDOWN, REENG_CEDO_DIAS, REENG_CEDO_COOLDOWN, REENG_CEDO_CHAVE
};
