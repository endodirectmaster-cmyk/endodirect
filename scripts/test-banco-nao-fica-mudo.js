...
(async function () {
  ...
  // Uma carga de conteúdo em voo por vez: quem chamar de novo recebe a mesma promessa.
  {
    ...
    ok(vm.runInContext('bancoEstado()', c) === 'carregando', 'em voo: o Banco diz "carregando" enquanto a carga não volta');
  ...
  // ── 4. Fiação no index.html (âncoras literais) ───────────────────────────
  {
    ...
    ok(/\.then\(function\(\)\{if\(remoteStateEmVoo&&remoteStateEmVoo\.p===emVoo\)remoteStateEmVoo=null;\}\)/.test(hyd), 'hydrate: solta a guarda ao terminar (sucesso ou falha)');
  ...
  }
  {
    ...
    ok(/remoteStateLoaded=false;hydrateRemoteState\(\);/.test(grant), 'checkout: liberado o acesso, aí sim a carga inteira');
  }
  {
    ...
    ok(/else if\(u\.id\)\{var _acG=acessosGuardados\(u\.id\);if\(_acG\)userAcessos=_acG;\}/.test(dl), 'doLogin: aluno real nasce com os acessos guardados neste aparelho');
  }
  {
    const upi = corpo('updateProvaInfo');
    ok(/var estado=bancoEstado\(\);/.test(upi), 'updateProvaInfo: consulta o estado do banco');
    ok(upi.indexOf("if(estado==='carregando')") < upi.indexOf('else if(isDegustacao()){'), 'updateProvaInfo: carregando/erro vêm ANTES da faixa de degustação (sem acessos, um Gold pareceria degustação)');
  ...
  // ── 5. O .sql versionado do member_content ──────────────────────────────
  {
    ...
    ok((sql.match(/radar_hidden/g) || []).length >= 2, 'sql: filtra radar_hidden e o devolve ao cliente');
